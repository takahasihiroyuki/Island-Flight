...
#include "graphics/Renderer/ModelRender.h"
...
namespace nsK2EngineLow {
	...
	)
	{
		...
		// GBuffer描画用のモデルを初期化
		InitModelOnRenderGBuffer(filePath, enModelUpAxis, isShadowReceiver);
	...
	void ModelRender::InitModelOnZprepass(const char* tkmFilePath, EnModelUpAxis modelUpAxis, bool isSkyCube)
	{
		...
		if (m_isEnableInstancingDraw) {
			...
			modelInitData.m_expandShaderResourceView[0] = &m_worldMatrixArraySB;
		}
	...
	void ModelRender::InitModelOnRenderGBuffer(const char* tkmFilePath, EnModelUpAxis enModelUpAxis, bool isShadowReceiver)
	{
		ModelInitData modelInitData;
		...
		if (isShadowReceiver) {
			modelInitData.m_psEntryPointFunc = "PSMainShadowReceiver";
		}
		else
		{
	...
	void ModelRender::InitShadowModel(const char* filePath, EnModelUpAxis enModelUpAxis)
	{
		...
		if (m_isEnableInstancingDraw) {
			// インスタンシング描画を行う場合は、拡張SRVにインスタンシング描画用のデータを設定する。
			shadowInitData.m_expandShaderResourceView[0] = &m_worldMatrixArraySB;
		}
	...
	void ModelRender::InitReflectionModel(const char* filePath, EnModelUpAxis enModelUpAxis, ReflectLayer layer)
	{
		...
		if (m_isEnableInstancingDraw) {
			// インスタンシング描画を行う場合は、拡張SRVにインスタンシング描画用のデータを設定する。
			reflectionInitData.m_expandShaderResourceView[0] = &m_worldMatrixArraySB;
		}
